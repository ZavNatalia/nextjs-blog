...
Это директива, которая **принудительно** делает страницу статической. Если во время рендеринга страницы Next.js обнаружит что-либо, требующее динамического рендеринга (например, `cookies()`, `headers()`, `fetch(..., { cache: 'no-store' })`, динамические параметры без `generateStaticParams()`), то произойдёт ошибка на этапе сборки.

### Для чего это нужно?

- Гарантирует, что страница полностью статическая (SSG)
- Отлично для SEO и максимально быстрой загрузки
- Помогает выявить неожиданные динамические зависимости на этапе разработки

### Альтернатива: `dynamic = 'error'`

Режим `'error'` также блокирует динамический рендеринг, но выбрасывает ошибку при любой попытке использовать динамические функции. Используйте `'force-static'` для явного указания статической генерации.
...
Эта директива вынуждает страницу **всегда рендериться на сервере при каждом запросе** – то есть включает Server-Side Rendering (SSR), даже если данные могли бы быть закэшированы или предрендерены.
...
- Нужен доступ к request-time данным (cookies, headers)
...
1. При первом запросе после истечения `revalidate` времени пользователь получает закэшированную версию
...
- Используется для интерактивных компонентов, которым не нужен SEO
...
- Компоненты, требующие browser API (localStorage, geolocation и т.д.)
...
- **Для интерактивных элементов** (комментарии, модалки) → используйте CSR
...
- `Suspense` позволяет Next.js отправить HTML части страницы немедленно
- Пользователь видит контент быстрее, даже если часть данных ещё загружается
- Отлично работает с SSR – страница остаётся SEO-friendly

### Преимущества Streaming:

- **Лучший UX**: пользователь видит контент быстрее
- **Гранулярный контроль**: можно оборачивать разные части страницы в отдельные `<Suspense>`
...
- [Гранулярный контроль стриминга](https://nextjs.org/learn/dashboard-app/streaming): оборачивайте отдельные компоненты для управления приоритетами загрузки
- **Loading UI**: используйте `loading.tsx` файлы для автоматических Suspense границ
...
✅ On-demand revalidation позволяет обновлять ISR страницы по событиям
